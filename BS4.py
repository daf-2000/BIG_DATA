...
for doc in db.collection.find():
    list_vacancy_url.append(doc['vacancy_url'])


for k in f:
    if k['vacancy_url'] not in list_vacancy_url:
        new_f.append(k)
    else:
        pass
...
if len(new_f) == 0:
    print('В БД вставленно 0 значений')
else:
    db.collection.insert_many(new_f)
    print('Вставлено новых значений {}'.format(len(new_f)))



def max_salary_get(max_s, db_collection):
    for i in db_collection.find({'salary_min': {'$gt': max_s}}):
        print(i)
max_salary_get(int(input('Введите минимальную ЗП для поиска: ')), db.collection)
